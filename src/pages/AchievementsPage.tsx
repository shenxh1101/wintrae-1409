...
const mapTypeMeta: Record<MapType, { name: string; icon: string; color: string; bg: string }> = {
  china: { name: '中国地图', icon: '🇨🇳', color: 'text-red-600', bg: 'bg-red-50 border-red-200' },
  world: { name: '世界大洲', icon: '🌍', color: 'text-blue-600', bg: 'bg-blue-50 border-blue-200' },
  grid: { name: '经纬度', icon: '🧭', color: 'text-green-600', bg: 'bg-green-50 border-green-200' },
  campus: { name: '校园平面', icon: '🏫', color: 'text-purple-600', bg: 'bg-purple-50 border-purple-200' },
}
...
const AchievementsPage: React.FC = () => {
  ...
  const byMapStats = useMemo(() => {
    const map: Record<MapType, { records: PlayRecord[]; wrongs: WrongQuestion[] }> = {
      china: { records: [], wrongs: [] },
      world: { records: [], wrongs: [] },
      grid: { records: [], wrongs: [] },
      campus: { records: [], wrongs: [] },
    }
    userData.playHistory.forEach(r => {
      if (map[r.mapType]) map[r.mapType].records.push(r)
    })
    userData.wrongQuestions.forEach(w => {
      if (map[w.question.mapType]) map[w.question.mapType].wrongs.push(w)
    })
    return map
  ...
  const recentAccuracy = useMemo(() => {
    ...
    return {
      value: Math.round(currAcc),
      trend: Math.abs(diff) < 1 ? 0 : (diff > 0 ? 1 : -1) as 0 | 1 | -1,
    }
  }, [accuracyTrend])
  ...
  const weakestTypes = useMemo(() => {
    ...
    return Array.from(cnt.entries())
      .map(([t, v]) => ({ type: t, label: typeLabel[t] ?? t, wrongCount: v.wrong, unique: v.total }))
        ...
        >
          ...
          <div className="grid grid-cols-4 gap-2 md:gap-3">
            <div className="bg-white/20 rounded-2xl p-3 text-center backdrop-blur-sm">
              ...
              <p className="text-xl md:text-2xl font-bold">{accuracy}%</p>
              <p className="text-xs text-white/80">总正确率</p>
            </div>
            <div className="bg-white/20 rounded-2xl p-3 text-center backdrop-blur-sm">
              <Clock className="w-5 h-5 mx-auto mb-1" />
              <p className="text-xl md:text-2xl font-bold">{avgTime}s</p>
              <p className="text-xs text-white/80">平均用时</p>
            </div>
            <div className="bg-white/20 rounded-2xl p-3 text-center backdrop-blur-sm">
              <Flame className="w-5 h-5 mx-auto mb-1" />
              <p className="text-xl md:text-2xl font-bold">{userData.streakDays}</p>
              <p className="text-xs text-white/80">连续天数</p>
            </div>
            <div className="bg-white/20 rounded-2xl p-3 text-center backdrop-blur-sm">
              <Calendar className="w-5 h-5 mx-auto mb-1" />
              <p className="text-xl md:text-2xl font-bold">{totalPlayCount}</p>
              <p className="text-xs text-white/80">练习次数</p>
            </div>
          </div>
        </motion.div>
      </header>
                    ...
                    >
                      ...
                      <div className="w-full h-3 bg-white/60 rounded-full overflow-hidden">
                        <motion.div
                          ...
                          className={`h-full rounded-full ${
                            s.accuracy >= 70 ? 'bg-green-500' : s.accuracy >= 40 ? 'bg-yellow-500' : 'bg-red-400'
                          }`}
                        />
                      </div>
                      <div className="flex justify-between text-xs text-gray-500 mt-2">
                        ...
                        <span className="text-red-500">
                          {s.wrongCount > 0 && `待复习错题 ${s.wrongCount} 道`}
                        </span>
                      </div>
                    </div>
                  )
              ...
              >
                ...
                <div className="space-y-3">
                  {weakestTypes.map((w, idx) => (
                    ...
                    >
                      <div className="w-8 h-8 rounded-full bg-red-500 text-white font-bold flex items-center justify-center text-sm">
                        {idx + 1}
                      </div>
                      <div className="flex-1">
                        <p className="font-bold text-red-700">{w.label}</p>
                        <p className="text-xs text-red-500">累计错误 {w.wrongCount} 次 · 涉及 {w.unique} 道题</p>
                      </div>
