...
import { ArrowLeft, Lightbulb, Clock, Star, Trophy, Home, RotateCcw, ChevronRight, Target, TrendingDown, CheckCircle2 } from 'lucide-react'
import { motion, AnimatePresence } from 'framer-motion'
import MapComponent from '../maps'
import { useGameStore } from '../store/gameStore'
import { useUserStore } from '../store/userStore'
import type { MapType, Difficulty, WrongQuestion, QuestionType, PracticeMode } from '../types'
...
const GamePage: React.FC = () => {
  ...
  useEffect(() => {
    if (gameOver && answerHistory.length > 0) {
  ...
  }, [showResult, gameOver, isPlaying, selectAnswer, timeLeft, difficulty, streak, currentQuestion, updateStats, addWrongQuestion, updateBestStreak, checkNewAchievements, isWrongReviewMode, markWrongMastered, updateWrongRecord])
  ...
  const handleRestart = () => {
    startGame(mapType, difficulty, 10, focusTypes)
    wrongIdSetRef.current = new Set()
    achievementSetRef.current = new Set()
    masteredThisRoundRef.current = new Set()
    setHintText(null)
  ...
  if (gameOver) {
    const { correctCount, bestStreak, accuracy, answeredCount } = finalStats
      ...
      <div className="min-h-screen p-4 md:p-8 flex items-center justify-center">
        ...
        >
          ...
          <h2 className="text-3xl font-bold text-gray-800 mb-2">
            {isWrongReviewMode ? '错题挑战完成！' : (focusTypes.length > 0 ? '专项训练完成！' : '挑战完成！')}
          </h2>
          ...
          <div className="grid grid-cols-2 gap-4 mb-6">
            ...
            <div className="bg-orange-50 rounded-2xl p-4">
              <p className="text-3xl font-bold text-orange-600">{correctCount}/{answeredCount || totalQuestions}</p>
                ...
                {perTypeStats.map(s => (
                  <div key={s.type}>
                    <div className="flex justify-between text-sm mb-1">
                      ...
                      <span className={s.accuracy >= 70 ? 'text-green-600 font-bold' : 'text-red-500 font-bold'}>
                        {s.correct}/{s.total} · {s.accuracy}%
                      </span>
                    </div>
                    <div className="w-full h-2 bg-gray-200 rounded-full overflow-hidden">
                      <div
                        className={`h-full rounded-full ${s.accuracy >= 70 ? 'bg-green-500' : 'bg-red-400'}`}
          ...
          <div className="flex flex-col gap-3">
            ...
            >
              ...
              {isWrongReviewMode ? '再练一轮错题' : focusTypes.length > 0 ? '再来一次专项' : '再来一局'}
            </button>
            <button
  ...
  const modeBadge = isWrongReviewMode
      ...
      <header className="max-w-4xl mx-auto mb-4">
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex items-center gap-3">
            {modeBadge && (
              <span className={`px-3 py-1 text-sm font-bold rounded-full ${modeBadge.cls}`}>
                {modeBadge.text}
              </span>
            )}
